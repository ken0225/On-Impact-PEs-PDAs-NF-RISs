...
% Initialize arrays to store SE values
SE_L_MC_values = zeros(1, num_b);
SE_U_MC_values = zeros(1, num_b);
SE_MC_values = zeros(1, num_b);
SE_L_AN_values = zeros(1, num_b);
SE_U_AN_values = zeros(1, num_b);
SE_AN_values = zeros(1, num_b);
SE_upper_AN_values = zeros(1, num_b);
SE_L_upper_AN_values = zeros(1, num_b);
SE_U_upper_AN_values = zeros(1, num_b);

%% Main Loop for different 'b' values
for b_idx = 1:num_b
    b = b_values(b_idx);
    disp(['------------ Current b value: ', num2str(b), ' ------------']);
    ...
    fprintf('         Generate the Rician channel. Progress:  40%%\n');
    ...
    SE_L_MC_values(b_idx) = mean(SE_L_MC); 

    %% SE_L AN
    disp('------------Step: Analytical Results SE_L calculation.------------');

    Gamma_SE_L = (((1-b)/2 .* sin(phi_L-c) .* (sin(x)/x) .* (besseli(1,kappa)./besseli(0,kappa)) + (1+b)/2) .* (besseli(1,kappa)./besseli(0,kappa)) .* (sin(x)/x)).^2;
    ...
    SE_L_AN_values(b_idx) = mean(SE_L_AN);

    %% SE_U MC
    disp('------------Step: SE_U MC calculation.------------');
    ...
    SE_U_MC_values(b_idx) = mean(SE_U_MC); 

     %% SE_U AN
    disp('------------Step: Analytical Results SE_U calculation.------------');

    Gamma_SE_U = (((1-b)/2 .* sin(phi_U-c) .* (sin(x)/x) .* (besseli(1,kappa)./besseli(0,kappa)) + (1+b)/2) .* (besseli(1,kappa)./besseli(0,kappa)) .* (sin(x)/x)).^2;
    ...
    SE_U_AN_values(b_idx) = mean(SE_U_AN);

    %% SE MC
    disp('------------Step: SE MC calculation.------------');
    ...
    SE_AN = log2(1+SNR);
    SE_AN_values(b_idx) = mean(SE_AN);

    %% SE_Upper AN
    disp('------------Step: Analytical Results SE_Upper calculation.------------');

    S_pixel_gain = [];
    S_star_all_T = [];

    Gamma_SE_star = (((1-b)/2 .* sin(phi_m-c) .* (sin(x)/x) .* (besseli(1,kappa)./besseli(0,kappa)) + (1+b)/2) .* (besseli(1,kappa)./besseli(0,kappa)) .* (sin(x)/x)).^2;

    for cc = 1 : T
        
        for dd = 1 : M
            
        [S_pixel_gain(dd)] = function_gain_each_pixel(D_AP, D_RIS_centers(dd, :, cc), d_x, d_y);
        
        end
        
        temp = Gamma_SE_star(cc,:).*S_pixel_gain;
        
        S_star_all_T(cc) = sum(temp);
        
        clear S_pixel_gain temp
        
        if mod(cc,ceil(T/10))==0 || cc==T
            fprintf('\rCompute E_star progress: %3.0f%%', cc/T*100);
        end
    end
    fprintf('\n');
    ...
    for cc = 1 : T
        ...
        [T_pixel_gain(dd)] = function_gain_each_pixel(D_User, D_RIS_centers(dd, :, cc), d_x, d_y);
        
        end
        
        T_star_all_T(cc) = sum(T_pixel_gain);
        
        clear T_pixel_gain
        
        if mod(cc,ceil(T/10))==0 || cc==T
            fprintf('\rCompute F_star progress: %3.0f%%', cc/T*100);
        end
    end
    fprintf('\n');

    h_abs_square_SE_star = (D_AP(3)*D_User(3))/(pi^2) .* S_star_all_T .* T_star_all_T;

    SNR_star = Pt_W*h_abs_square_SE_star/sigma_square_W;

    SE_upper_AN = log2(1+SNR_star);
    SE_upper_AN_values(b_idx) = mean(SE_upper_AN);

    %% SE_L_Upper and SE_U_Upper AN
    disp('------------Step: Analytical Results SE_L_Upper, SE_U_Upper calculation.------------');
    ...
    for cc = 1 : T
        ...
        S_star_all_T(cc) = sum(S_pixel_gain);
        
        clear S_pixel_gain
        
        if mod(cc,ceil(T/10))==0 || cc==T
            fprintf('\rCompute E_star (L/U) progress: %3.0f%%', cc/T*100);
        end
    end
    fprintf('\n');
    ...
    for cc = 1 : T
        ...
        if mod(cc,ceil(T/10))==0 || cc==T
            fprintf('\rCompute F_star (L/U) progress: %3.0f%%', cc/T*100);
        end
    end
    fprintf('\n');

    Gamma_SE_L_star = (((1-b)/2 .* sin(phi_L-c) .* (sin(x)/x) .* (besseli(1,kappa)./besseli(0,kappa)) + (1+b)/2) .* (besseli(1,kappa)./besseli(0,kappa)) .* (sin(x)/x)).^2;

    h_abs_square_SE_L_star = Gamma_SE_L_star .* (D_AP(3)*D_User(3))/(pi^2) .* S_star_all_T .* T_star_all_T;

    SNR_L_star = Pt_W*h_abs_square_SE_L_star/sigma_square_W;

    SE_L_upper_AN = log2(1+SNR_L_star);
    SE_L_upper_AN_values(b_idx) = mean(SE_L_upper_AN);

    Gamma_SE_U_star = (((1-b)/2 .* sin(phi_U-c) .* (sin(x)/x) .* (besseli(1,kappa)./besseli(0,kappa)) + (1+b)/2) .* (besseli(1,kappa)./besseli(0,kappa)) .* (sin(x)/x)).^2;

    h_abs_square_SE_U_star = Gamma_SE_U_star .* (D_AP(3)*D_User(3))/(pi^2) .* S_star_all_T .* T_star_all_T;

    SNR_U_star = Pt_W*h_abs_square_SE_U_star/sigma_square_W;

    SE_U_upper_AN = log2(1+SNR_U_star);
    SE_U_upper_AN_values(b_idx) = mean(SE_U_upper_AN);
...
plot(b_values, SE_U_MC_values, ':', 'LineWidth', 3.5, 'Color', [244, 177, 131] ./ 255, 'DisplayName', '$\mathsf{SE}_\mathsf{U}$');
plot(b_values, SE_MC_values, '-.', 'LineWidth', 3.5, 'Color', [128, 116, 200] ./ 255, 'DisplayName', '$\mathsf{SE}$');
plot(b_values, SE_L_MC_values, '-', 'LineWidth', 3.5, 'Color', [169, 209, 142] ./ 255, 'DisplayName', '$\mathsf{SE}_\mathsf{L}$');

xlabel('$b\in[0.2, 1]$','Interpreter','LaTex');
