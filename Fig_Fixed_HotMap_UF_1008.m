...
%% Gamma Calculation for Different tau Values
tau_values = [pi/2, pi/4, pi/8];

% Preallocate to store Gamma values
Gamma_all = cell(size(tau_values));

% Calculate Gamma values for each tau
for k = 1:length(tau_values)
    tau = tau_values(k); % Current tau value
...
% Find min and max Gamma values across all tau values
min_Gamma = inf;
max_Gamma = -inf;
...
for k = 1:length(tau_values)
    ...
    % Create a new figure for each tau
    figure(k+4);  % Start figure numbers from 5 to avoid overwriting
    ...
    title(['Heatmap for tau = ' num2str(tau)]);
