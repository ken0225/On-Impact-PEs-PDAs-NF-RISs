...
a = 1; 
b = 0.2; 
c_values = [0.2*pi, 0.4*pi, 0.6*pi, 0.8*pi, pi]; 
num_c = length(c_values);

SE_L_MC_values = zeros(1, num_c);
SE_U_MC_values = zeros(1, num_c);
SE_MC_values = zeros(1, num_c);

%% Main Loop for different 'c' values
for c_idx = 1:num_c
    c = c_values(c_idx);
    disp(['------------ Current c value: ', num2str(c), ' ------------']);
    ...
    delta_m = random('Uniform', -tau_pe, tau_pe, T, M, realization);
    fprintf('         Generate UF noise (bar_delta_m): 20%%\n');

    bar_delta_m = random('Uniform', -tau_pe, tau_pe, T, M, realization);
    ...
    SE_L_MC_values(c_idx) = mean(SE_L_MC); 
    ...
    SE_U_MC_values(c_idx) = mean(SE_U_MC); 
    ...
    for aa = 1 : realization
        fprintf('\rSE MC progress: %3.0f%%', (aa-1)/realization*100);
...
p1=plot(c_values, SE_U_MC_values, ':', 'LineWidth', 3.5, 'Color', [244, 177, 131] ./ 255, 'DisplayName', '$\mathsf{SE}_\mathsf{U}$');
p2=plot(c_values, SE_MC_values, '-.', 'LineWidth', 3.5, 'Color', [128, 116, 200] ./ 255, 'DisplayName', '$\mathsf{SE}$');
p3=plot(c_values, SE_L_MC_values, '-', 'LineWidth', 3.5, 'Color', [169, 209, 142] ./ 255, 'DisplayName', '$\mathsf{SE}_\mathsf{L}$');

xlabel('$c\in[0.2\pi, \pi]$','Interpreter','LaTex');
ylabel('$\mathsf{SE}$ (bit/s/Hz)','Interpreter','LaTex');
legend([p1, p2, p3],'Interpreter','LaTex','Location','SouthEast');
