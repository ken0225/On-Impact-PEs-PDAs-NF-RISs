...
%% Practical Phase Shift Model Visualization (Varying c values, Final Version)
% Modified style to handle custom legend display for "pi" coefficients
...
b = 0.2;
% c values to scan
c_values = [0.5*pi, pi, 1.5*pi, 2*pi];
% Color list
colors = {[169, 209, 142]./255, [244, 177, 131]./255, [231, 98, 84]./255, [177, 177, 177]./255}; 
...
% Loop through different 'c' values to plot curves
for i = 1:length(c_values)
    ...
    % --- Dynamic generation of legend labels ---
    c_factor = c/pi;
    if c_factor == 1
        % If c is pi, do not display the coefficient "1"
        displayName = sprintf('$c=\\pi$ (Area = %.3f)', area_z);
    else
        % For other values (0.5, 1.5, 2), display coefficient normally
        displayName = sprintf('$c=%g\\pi$ (Area = %.3f)', c_factor, area_z);
    end
    
    % Plot curves
    plot(real(z), imag(z), 'Color', colors{i}, 'LineWidth', 3, 'DisplayName', displayName);
end
...
title(sprintf('$a=%.0f, b=%.1f$', a, b), 'Interpreter', 'latex');

% Configure legend and axis properties
