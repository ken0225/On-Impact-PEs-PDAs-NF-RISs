function [tfVMVariates] = function_vmrand(mu, kappa, varargin)
...
vbScalarArgs = [isscalar(mu) isscalar(kappa)];

if (nnz(vbScalarArgs) == 1)
   if vbScalarArgs(1)
      % - fMu is scalar
      mu = repmat(mu, size(kappa));
   else
      % - fKappa is scalar
      kappa = repmat(kappa, size(mu));
   end
   
   % - Set return sizes
   vnTensorSize = size(mu);
   
elseif (nnz(vbScalarArgs == 0))
   % - Two non-scalar arguments
   if (~isequal(size(mu), size(kappa)))
...
elseif (~isempty(varargin))
   ...
   kappa = repmat(kappa, vnTensorSize);
...
if (all(vbScalarArgs))
   tfTau = sqrt(4 .* kappa(1).^2 + 1) + 1;
   tfRho = (tfTau - sqrt(2 .* tfTau)) ./ (2 .* kappa(1));
   tfR = repmat((1 + tfRho.^2) ./ (2 .* tfRho), vnTensorSize);

else
   tfTau = sqrt(4 .* kappa.^2 + 1) + 1;
   tfRho = (tfTau - sqrt(2 .* tfTau)) ./ (2 .* kappa);
...
while (nnz(tbDraw > 0))
   ...
   tfC(tbDraw) = kappa(tbDraw) .* (tfR(tbDraw) - tfF(tbDraw));
