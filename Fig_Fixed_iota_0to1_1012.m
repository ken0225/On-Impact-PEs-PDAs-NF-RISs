...
[kappa_d, rho_m, kappa_m] = function_Rician_factor(d_0, d_AP_RIS, d_RIS_User, rician_factor_1, rician_factor_2);
...
iota_values = 0:0.05:1; 
num_iota = length(iota_values);

SE_L_MC_all = zeros(1, num_iota);
SE_U_MC_all = zeros(1, num_iota);
SE_MC_all = zeros(1, num_iota);
...
for iota_idx = 1:num_iota
    iota = iota_values(iota_idx);

% ========================= Prop 3.10 =========================
fprintf('         Generate noises for iota = %.1f: 0%%\n', iota);

delta_m = random('Uniform', -tau, tau, T, M, realization);
breve_delta_m = random('Uniform', -tau, tau, T, M, realization);
gamma_m = function_vmrand(mu, kappa, T, M, realization);
breve_gamma_m = function_vmrand(mu, kappa, T, M, realization); 

bar_delta_m = iota * delta_m + sqrt(1 - iota^2) * breve_delta_m;
bar_gamma_m = iota * gamma_m + sqrt(1 - iota^2) * breve_gamma_m;

fprintf('         Generate noises for iota = %.1f: 100%%\n', iota);
    ...
    SE_L_MC_all(iota_idx) = mean(SE_L_MC); 
    ...
    [SE_U_MC, ~] = function_compute_SE(signal_all_realization, Pt_W, sigma_square_W, kappa_t, kappa_r);
     SE_U_MC_all(iota_idx) = mean(SE_U_MC); 
...
marker_index = 1 : 3 : length(iota_values); 

hold on; box on; grid on;

p1 = plot(iota_values, SE_U_MC_all, ':', 'LineWidth', 3.5, 'Color', [244, 177, 131] ./ 255); 
p2 = plot(iota_values, SE_MC_all, '-.', 'LineWidth', 3.5, 'Color', [128, 116, 200] ./ 255); 
p3 = plot(iota_values, SE_L_MC_all, '-', 'LineWidth', 3.5, 'Color', [169, 209, 142] ./ 255); 
...
'$\mathsf{SE}_\mathsf{U}$',...
'$\mathsf{SE}$',...
'$\mathsf{SE}_\mathsf{L}$',...
